...
/* harmony export */   "initializeValidateCommonWithBraintree": function() { return /* binding */ initializeValidateCommonWithBraintree; }
/* harmony export */ });
/* unused harmony exports initializeGlobalValidateEventMap, initializeModalDialogIframe, initializeExpressCheckout, validateInputRules, validateComboboxRules, validateTextareaRules, ValidateEventHTTPResponseType, initializeValidateEventWithAjaxSubmit, processRequestByFormAction, validatePromoCodeWhenVisible, initializeValidateEventWithPromoCode, initializeValidateCommon, initializeValidateCommonWithActionCodeRadioGroup, initializeQuestionCodeCheckboxes, initializeValidateCommonWithStripe, getHiddenFormControlsByElement, addAttributeToFormControls, removeAttributeFromFormControls, validateByElementController, initializeValidateEventNavigator, updateFieldsetViewByElementController, validateByNavigator, initializeValidateEventThenSubscribe, relocateThenValidateUpsellActionQuantities, validatePromoCodeAroundStatusScreen */
...
var initializeValidateEventNavigator = function initializeValidateEventNavigator(form, submit, afterInitAction) {
  if (afterInitAction === void 0) {
    afterInitAction = function afterInitAction() {};
  }
  ...
  afterInitAction({
    ...
    nextNavigators: nextNavigators,
    prevNavigators: prevNavigators
  });
  return new Promise(function (resolve, reject) {
...
/* harmony import */ var Shared_ts_api_express_checkout_express_checkout__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! Shared/ts/api/express-checkout/express-checkout */ "../../Shared-International/ts/api/express-checkout/express-checkout.ts");
/* harmony import */ var Shared_ts_applications_form__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! Shared/ts/applications/form */ "../../Shared-International/ts/applications/form.ts");
/* harmony import */ var Shared_ts_components_status_screen__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! Shared/ts/components/status-screen */ "../../Shared-International/ts/components/status-screen.ts");
...
/* harmony import */ var Shared_ts_utils_validate_event__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! Shared/ts/utils/validate-event */ "../../Shared-International/ts/utils/validate-event.ts");
...
var validateCommonWhenAvailable = function validateCommonWhenAvailable() {
    var form = document.querySelector("[data-validate-common-form]");
    if (!form)
        return;
    (0,Shared_ts_applications_form__WEBPACK_IMPORTED_MODULE_9__.initializeValidateCommonWithBraintree)(form);
};
...
var initializeValidateSearchForm = function initializeValidateSearchForm() {
    var _validateSearchForm$s, _validateSearchForm$s2, _validateSearchForm$s3;
    ...
    if (!submit)
        return;
    var validateSearchForm = new Shared_ts_utils_validate_event__WEBPACK_IMPORTED_MODULE_12__["default"]({
        form: form,
        submit: submit,
        invalidInputEvents: ["keyup", "blur", "change"]
    });
    (_validateSearchForm$s = validateSearchForm.submit) == null ? void 0 : _validateSearchForm$s.addEventListener("click", function (event) {
        return validateSearchForm.validateAll();
    });
    validateSearchForm.processInputEvents();
    (_validateSearchForm$s2 = validateSearchForm.submit) == null ? void 0 : _validateSearchForm$s2.addEventListener("click", function (event) {
        return validateSearchForm.validateAll();
    });
    var text = validateSearchForm.inputs.find(function (input) {
        return input.id === "text";
    });
    if (!text)
        return;
    var textS = validateSearchForm.inputs.find(function (input) {
        return input.id === "textS";
    });
    if (!textS)
        return;
    textS.addEventListener("keyup", function (event) {
        return text.value = textS.value;
    });
    (_validateSearchForm$s3 = validateSearchForm.submit) == null ? void 0 : _validateSearchForm$s3.addEventListener("submit", function (event) {
        return text.value.replace(/^\s+|\s+$/, "").length !== 0;
    });
};
initializeValidateSearchForm();
var initializeProductDetailPageValidation = function initializeProductDetailPageValidation() {
    var form = document.querySelector("form.product__info");
    if (!form)
        return;
    var termsCheckbox = form.querySelector("input[type=checkbox][id^=terms-][id$=-PDP]");
    if (!termsCheckbox)
        return;
    var productCode = termsCheckbox.getAttribute("data-product-code");
    if (!productCode)
        return;
    var subscribeOption = form.querySelector("input[type=\"radio\"][value=\"" + productCode + "\"]");
    if (!subscribeOption)
        return;
    var addCartId = termsCheckbox.getAttribute("aria-controls");
    if (!addCartId)
        return;
    var addCartButton = form.querySelector("button#" + addCartId);
    if (!addCartButton)
        return;
    var validateCondition = function validateCondition() {
        termsCheckbox.checked && subscribeOption.checked ? addCartButton.removeAttribute("disabled") : addCartButton.setAttribute("disabled", "true");
    };
    ...
    var ve = (0,Shared_ts_applications_form__WEBPACK_IMPORTED_MODULE_9__.initializeValidateEvent)(form, addCartButton, "required");
    addCartButton.addEventListener("mouseover", function (event) {
        return ve.validateAll();
    });
};
initializeProductDetailPageValidation();
(0,Shared_ts_observers_intersection__WEBPACK_IMPORTED_MODULE_11__.observer)("[data-validate-form]", {
    inRange: function inRange(element) {
        var _validateMessage$subm;
        var validateMessage = new Shared_ts_utils_validate_event__WEBPACK_IMPORTED_MODULE_12__["default"]({
            form: element,
            submit: element.querySelector("[data-validate-form-type=\"submit\"]"),
            attribute: "data-required",
            inputEvents: ["blur", "change"],
            invalidInputEvents: ["keyup", "blur", "change"],
            comboboxEvents: ["change", "blur"]
        });
        validateMessage.validateInputEvent = function (event) {
            validateMessage.validateControl(event.target, validateInputRules);
        };
        validateMessage.validateComboboxEvent = function (event) {
            validateMessage.validateControl(event.target, validateComboboxRules);
        };
        (_validateMessage$subm = validateMessage.submit) == null ? void 0 : _validateMessage$subm.addEventListener("click", function (event) {
...
addEventListener("ECDrawFormComplete", function (event) {
    var checkout = document.querySelector("[data-validate-common-form]");
    if (!checkout)
        return;
    var ec = new Shared_ts_api_express_checkout_express_checkout__WEBPACK_IMPORTED_MODULE_8__["default"](checkout);
    ec.createCheckoutButtonGUI();
});
var validatePromoCodeAroundStatusScreen = function validatePromoCodeAroundStatusScreen() {
    var _validateEvent$submit;
    ...
    if (!statusContainer)
        return;
    var statusScreen = new Shared_ts_components_status_screen__WEBPACK_IMPORTED_MODULE_10__["default"]("inline", statusContainer);
    var wasValidated = false;
    var capturePromoCode;
    var validateEvent = (0,Shared_ts_applications_form__WEBPACK_IMPORTED_MODULE_9__.initializeValidateEvent)(form, form.querySelector('button[type="button"]'));
    var input = validateEvent.inputs.find(function (input) {
        return input.id === "promoCode";
    });
    input == null ? void 0 : input.addEventListener("keyup", function (event) {
    ...
    (0,Shared_ts_applications_form__WEBPACK_IMPORTED_MODULE_9__.validatePromoCode)().pass(function (promoCode) {
        statusScreen.done("Code " + promoCode + " was successfully redeemed.");
    }).fail(function (error) {
        statusScreen.problem(error);
    }).finally(function () {
