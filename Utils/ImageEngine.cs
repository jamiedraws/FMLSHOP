...
namespace FMLSHOP.Utils
{
    public class ImageEngine
    {
        ...
        public string BuildHTMLPictureElement(string file, bool lazyLoad = false)
        {
            ...
            string description = GetImageMetaDescriptionByBitmap(bitmap);

            string imageAttributeList = string.Format(@"src=""{0}"" alt=""{1}"" width=""{2}"" height=""{3}""", imageSource, description, bitmap.Width, bitmap.Height);
